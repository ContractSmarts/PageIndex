...
import tiktoken
import openai
from openai import AzureOpenAI, AsyncAzureOpenAI # MODIFICATION: Azure imports
import logging
import os
...
# MODIFICATION: Global Client Definitions
# This satisfies the scope for all functions below
client = AzureOpenAI(
...
def count_tokens(text, model=None):
    if not text:
        return 0
    # Standardize to cl100k_base for GPT-4 family
    try:
...
def ChatGPT_API_with_finish_reason(model, prompt, api_key=None, chat_history=None):
    """MODIFICATION: Uses global Azure client and deployment name"""
    max_retries = 10
    for i in range(max_retries):
        try:
            ...
            print(f'*** Azure Sync Retry ({i}): {e} ***')
            time.sleep(1)
    return "Error", "failed"

def ChatGPT_API(model, prompt, api_key=None, chat_history=None):
    """MODIFICATION: Uses global Azure client"""
    max_retries = 10
    for i in range(max_retries):
        try:
            messages = chat_history.copy() if chat_history else []
...
async def ChatGPT_API_async(model, prompt, api_key=None):
    """MODIFICATION: Uses global Azure async_client"""
    max_retries = 10
    for i in range(max_retries):
        try:
